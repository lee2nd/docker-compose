...
def job():
    ...
    logging.basicConfig(filename=f'log/l6b_fs_eb16c_sup_{datetime.today().strftime("%Y_%m_%d_%H_%M")}.log', filemode='w+', format='%(name)s - %(levelname)s - %(message)s', level=logging.INFO)
    
    for file in os.listdir(f"../AT_DATA/l6b_fs_eb16c/file_log"):
        
        file_log_path = os.path.join(f"../AT_DATA/l6b_fs_eb16c/file_log", file)
                ...
                if time_difference.total_seconds()/60 <= trigger_setting.time_difference:
                    ...
                    if len(lot_id) == 10:
                        ...
                        if "Adr" in file_name:
                            ...
                            try:
                                f = open(f"../AT_DATA/l6b_fs_eb16c/{file_path}", 'r')
                            except:
                                continue
                            lines = f.readlines()
                        ...
                        # Chargemap 2d 解檔
                            ...
                            try:
                                ...
                                charge_2d_r, charge_2d_g, charge_2d_b = process_charge_file(f"../AT_DATA/l6b_fs_eb16c/{file_path}", lot_id)
