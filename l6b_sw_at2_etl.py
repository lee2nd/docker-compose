...
fs_charge2d = gridfs.GridFS(db, collection="L6B_SW_AT2_charge2d")
fs_defectinfo = gridfs.GridFS(db, collection="L6B_SW_AT2_defectinfo")
...
def job():
    ...
    logging.basicConfig(filename=f'log/l6b_sw_at2_sup_{datetime.today().strftime("%Y_%m_%d_%H_%M")}.log', filemode='w+', format='%(name)s - %(levelname)s - %(message)s', level=logging.INFO)
    
    for file in os.listdir(f"../AT_DATA/l6b_sw_at2/file_log"):
        
        file_log_path = os.path.join(f"../AT_DATA/l6b_sw_at2/file_log", file)
        ...
        if all([cond_1_1,cond_1_2,cond_1_3]):
            ...
            for i in range(1,len(fileloglines)):
                ...
                if time_difference.total_seconds()/60 <= trigger_setting.time_difference:
                    ...
                    eqp_id = filelog[1]
                    recipe_id = filelog[2]
                    if filelog[6][0] != 'E': continue
                    chip_id = filelog[6]
                    if "_" in chip_id: chip_id = chip_id.split("_")[0]
                    sheet_id = chip_id[:-5] + "P" + chip_id[-4:-2]
                    file_name = filelog[8]
                    file_path = filelog[9]
                    file_path = file_path.replace("\\","/")[13:]
                    ins_cnt = filelog[10].replace("\n", "")
                    ...
                    if "Adr" in file_name:
                        ...
                        try:
                            f = open(f"../AT_DATA/l6b_sw_at2/{file_path}", 'r')
                        except:
                            continue
                        lines = f.readlines()
                    ...
                    # Chargemap 2d 解檔
                        ...
                        step = filelog[8].split("\\")[-1].split("_")[1] + "_" + filelog[8].split("\\")[-1].split("_")[2].split(".")[0]
                        charge_type = filelog[8].split(".")[-1]
                        ...
                        try:
                            ...
                            charge_2d_r, charge_2d_g, charge_2d_b = process_charge_file(f"../AT_DATA/l6b_sw_at2/{file_path}", sheet_id)
