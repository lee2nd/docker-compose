...
# 資料庫連結
db = client["AT"]
collection_charge2d = db["L6B_SW_TC01_charge2d"]
collection_defectinfo = db["L6B_SW_TC01_defectinfo"]
fs_charge2d = gridfs.GridFS(db, collection="L6B_SW_TC01_charge2d")
fs_defectinfo = gridfs.GridFS(db, collection="L6B_SW_TC01_defectinfo")
...
def process_charge_file(file_path, sheet_id):
    
    filetype = file_path.split('.')[-1]     
    sheet_id_f2 = sheet_id[:2]

    W = df_config[df_config["model"]==sheet_id_f2]["W"].values[0]
    H = df_config[df_config["model"]==sheet_id_f2]["H"].values[0]      
...
def job():
    ...
    logging.basicConfig(filename=f'log/l6b_sw_tc01_sup_{datetime.today().strftime("%Y_%m_%d_%H_%M")}.log', filemode='w+', format='%(name)s - %(levelname)s - %(message)s', level=logging.INFO)
    
    for file in os.listdir(f"../AT_DATA/l6b_sw_tc01/file_log"):
        
        file_log_path = os.path.join(f"../AT_DATA/l6b_sw_tc01/file_log", file)
        filename = file_log_path.split("/")[-1]
        
        try:
            filedate = datetime.strptime(filename[5:13], "%Y%m%d")
        except:
            continue
        
        cond_1_1 = (filename[:4] == "TC01")
        cond_1_2 = (len(filename) == 17)
            ...
            for i in range(1,len(fileloglines)):
                ...
                if time_difference.total_seconds()/60 <= trigger_setting.time_difference:
                    ...
                    if "Adr" in file_name:
                        ...
                        try:
                            f = open(f"../AT_DATA/l6b_sw_tc01/{file_path}", 'r')
                        except:
                            continue
                        lines = f.readlines()
                        
                        defect_info = []
                        color = {0:df_config[df_config["model"]==sheet_id[:2]]["RGB order"].values[0][2],
                                1:df_config[df_config["model"]==sheet_id[:2]]["RGB order"].values[0][0],
                                2:df_config[df_config["model"]==sheet_id[:2]]["RGB order"].values[0][1]}
                        Defect_code = ""
                    ...
                    # Chargemap 2d 解檔
                        ...
                        step = filelog[10].split("\\")[-1].split("_")[1] + "_" + filelog[10].split("\\")[-1].split("_")[2].split(".")[0]
                        charge_type = filelog[10].split(".")[-1]
                        ...
                        try:
                            ...
                            charge_2d_r, charge_2d_g, charge_2d_b = process_charge_file(f"../AT_DATA/l6b_sw_tc01/{file_path}", sheet_id)
