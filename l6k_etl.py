...
def job():
    ...
    logging.basicConfig(filename=f'AT_CODE_SUP/log/l6k_sup_{datetime.today().strftime("%Y_%m_%d_%H_%M")}.log', filemode='w+', format='%(name)s - %(levelname)s - %(message)s', level=logging.INFO)

    adr_paths = glob.glob("AT_DATA/l6k_sup/*/ADR/*/*")
    chargemap_dict = dict()
    ...
    for adr_path in adr_paths:
        ...
        if adr_path[-3:] == "Adr":
            ...
            print(adr_path)
            logging.info(adr_path)
            f = open(adr_path, 'r')
            lines = f.readlines()
            model = adr_path.split("/")[2][:2]
            ...
            with open("l6k_chargemap.json", "w") as file:
                json.dump({}, file)  # 清空檔案內容            
            with open("l6k_chargemap.json", "w") as file:
                json.dump(chargemap_dict, file)            

    with open("l6k_chargemap.json", "r") as file:
